...
ggsave(filename = file.path("pie9.png"),, plot = pie9, width = 8, height = 6, dpi = 600)
...
ggsave(filename = file.path("pie10.png"), plot = pie10, width = 8, height = 6, dpi = 600)
...
ggsave(filename = file.path("pie11.png"), plot = pie11, width = 8, height = 6, dpi = 600)
...
head(sorted_df2)
Plot5 <- ggplot(new_df2, aes(x = "", y = total_percentage, fill = Hit)) +
  ...
  theme(
    ...
    legend.title = element_blank(),
    legend.text = element_text(size = 4, color = "#000000"),  # Smaller text size and color
    legend.key.size = unit(0.5, "cm"),  # Make the legend keys (colored boxes) smaller
    legend.key.height = unit(0.3, "cm"),  # Adjust height of the key
