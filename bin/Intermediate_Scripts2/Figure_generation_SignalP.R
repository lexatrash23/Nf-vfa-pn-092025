...
# percentage of transcripts with no hit vs hit
...
pie9 <- ggplot(counts, aes(x = "", y = Count, fill = Category)) +
  geom_bar(stat = "identity", width = 1, color = "black") +
  coord_polar(theta = "y", start = 0) +  # Set the starting angle for the slices
  theme_void() +  # Removes axes and background
  labs(title = "% of transcripts with a significant hit to a uniprot toxin (Evalue cutoff:1e-5)") +
...
ggsave(filename = file.path("pie9.png"),, plot = pie9, width = 8, height = 6, dpi = 600, bg = "white")
...
ggsave("alluvial5_plot.png", plot = plot_nolegend, width = 8, height = 6, dpi = 600, bg = "white")
ggsave("alluvial5_legend.png", legend_plot,width = w + 0.2,height = h + 0.2, dpi = 600, bg = "white")
...
plot_nolegend <- alluvial6 + theme(legend.position = "none")
legend <- get_legend(alluvial6)
legend_plot <- ggdraw(legend)
w <- convertWidth(sum(legend$widths), "in", valueOnly = TRUE)
h <- convertHeight(sum(legend$heights), "in", valueOnly = TRUE)

ggsave("alluvial6_plot.png", plot = plot_nolegend, width = 8, height = 6, dpi = 600, bg = "white")
ggsave("alluvial6_legend.png", legend_plot,width = w + 0.2,height = h + 0.2, dpi = 600, bg = "white")
...
# Plot the pie chart
pie10 <- ggplot(pie_data, aes(x = "", y = Value, fill = Category)) +
  geom_bar(stat = "identity", width = 1, color = "black") +
  coord_polar(theta = "y", start = 0) +  # Set the starting angle for the slices
  theme_void() +  # Removes axes and background
...
ggsave(filename = file.path("pie10.png"), plot = pie10, width = 8, height = 6, dpi = 600, bg = "white")
...
#  pie chart
pie11 <- ggplot(pie_data2, aes(x = "", y = Value, fill = Category)) +
  geom_bar(stat = "identity", width = 1, color = "black") +
  coord_polar(theta = "y", start = 0) +  # Set the starting angle for the slices
  theme_void() +  # Removes axes and background
  ...
  scale_fill_manual(values = c("With Hits" = "#4C9E9A", "Without Hits" = "#B0B0B0")) +
  theme(
    plot.title = element_text(size = 10, face = "bold", hjust = 0.2, color = "black"),  # Center title with nice font
    legend.position = "right",  # Position the legend on the right
    legend.title = element_blank(),
    legend.text = element_text(color = "black"),
    plot.margin = margin(20, 20, 20, 20)  # Add padding around the plot
...
ggsave(filename = file.path("pie11.png"), plot = pie11, width = 8, height = 6, dpi = 600, bg = "white")
...
write.csv(top10mostexpressed,file = file.path("Table15.csv"), row.names = FALSE)
...
Plot5 <- ggplot(Distinct_Transcripts_50_with_kallisto_mod, aes(x = 2, y = RelativeProportion, fill = fct_inorder(Hit))) +
...
plot_nolegend <- Plot5 + theme(legend.position = "none")
legend <- get_legend(Plot5)
legend_plot <- ggdraw(legend)
w <- convertWidth(sum(legend$widths), "in", valueOnly = TRUE)
h <- convertHeight(sum(legend$heights), "in", valueOnly = TRUE)

ggsave("pie12_plot.png", plot = plot_nolegend, width = 8, height = 6, dpi = 600, bg = "white")
ggsave("pie12_legend.png", legend_plot,width = w + 0.2,height = h + 0.2, dpi = 600, bg = "white")
