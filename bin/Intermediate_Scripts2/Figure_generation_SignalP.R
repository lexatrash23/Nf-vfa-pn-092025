...
keeps <- c("Transdecoder_ID", "ORF_type", "SP_Prediction","percent", "cumulativepercent", "Code", "Hit", "BitScore")
transdf <- transdf[keeps]
head(transdf)
transdf <- transdf[
transdf$SP_Prediction == "SP(Sec/SPI)" &
  !is.na(transdf$SP_Prediction) &
...
alluvial6 <- ggplot(data = Distinct_Transcripts_300,
                    aes(axis1 =Transdecoder_ID , axis2 = Hit)) +
  geom_alluvium(aes(fill = Hit)) +
  geom_stratum(aes(fill = Hit)) +
  geom_text(stat = "stratum",
